...
export default async function CustomerWorkspacePage({ searchParams }: { searchParams: Promise<{ organizationId?: string; branchId?: string; period?: string }> }) {
  ...
  return <CustomerWorkspaceScreen organizationId={selected.id} branchId={branchId} period={period} identityId={identity.id} identityLabel={identity.displayName} signOutHref={signOutPath("/")} />;
}
