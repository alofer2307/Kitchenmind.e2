...
export default async function PlatformPage() {
  ...
  const signOutHref = chatGPTSignOutPath("/platform");
...
function PlatformAccessState({ mode }: { mode: "unauthenticated" | "unauthorized" | "unavailable" }) {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-[#f4f8fc] p-5"><Card className="w-full max-w-md rounded-[1.75rem] border-[#d5e5ef] shadow-[0_24px_70px_rgba(24,52,81,.1)]"><CardContent className="p-7 text-center sm:p-9"><div className="flex justify-center"><BrandMark /></div><div className="mx-auto mt-8 flex size-12 items-center justify-center rounded-2xl bg-[#e8f2ff] text-[#1769e0]"><Icon className="size-6" /></div><h1 className="mt-5 text-2xl font-black tracking-[-0.03em] text-[#102b48]">{content.title}</h1><p className="mt-3 text-sm leading-6 text-[#647a90]">{content.description}</p>{mode === "unauthenticated" ? <Button asChild className="mt-6 h-11 w-full rounded-xl"><a href={chatGPTSignInPath("/platform")} target="_top">Iniciar sesión segura</a></Button> : <Button asChild variant="outline" className="mt-6 h-11 w-full rounded-xl"><Link href="/">Volver a KitchenMind</Link></Button>}</CardContent></Card></main>;
}
