...
export default async function ProductEntryPage() {
  ...
  if (!identity) return <ProductAccessScreen mode="signed_out" signInHref={chatGPTSignInPath("/")} />;
  ...
  try {
    ...
    return <ProductAccessScreen mode="unavailable" identityLabel={identity.displayName} signOutHref={chatGPTSignOutPath("/")} />;
  }
  ...
  if (activeMemberships.length === 1) {
    ...
    return <ProductAccessScreen mode="restricted" identityLabel={identity.displayName} organizationName={organization.name} signOutHref={chatGPTSignOutPath("/")} />;
  }
  const suspendedMembership = access.organizations.find((organization) => organization.membershipStatus === "suspended");
  if (suspendedMembership) return <ProductAccessScreen mode="suspended" identityLabel={identity.displayName} organizationName={suspendedMembership.name} signOutHref={chatGPTSignOutPath("/")} />;
  return <ProductAccessScreen mode="no_membership" identityLabel={identity.displayName} signOutHref={chatGPTSignOutPath("/")} />;
}
